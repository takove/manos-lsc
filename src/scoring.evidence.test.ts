...
export const transform = (frames: Frame[], scale = 1, dx = 0, dy = 0) =>
  frames.map((frame) => ({
    ...frame,
    ...Object.fromEntries(
      (["p", "l", "r"] as const).map((side) => [
        side,
        frame[side].map((p) => ({
          ...p,
          x: p.x * scale + dx,
...
export const reverse = (frames: Frame[]) =>
  copy(frames)
    .reverse()
    .map((frame, i) => ({ ...frame, t: frames[i].t }));
export const still = (frames: Frame[]) =>
  frames.map((frame) => ({
    ...structuredClone(frames[Math.floor(frames.length / 2)]),
    t: frame.t,
  }));
export const shiftHands = (frames: Frame[], dy: number) =>
  frames.map((frame) => ({
    ...frame,
    l: frame.l.map((p) => ({ ...p, y: p.y + dy })),
    r: frame.r.map((p) => ({ ...p, y: p.y + dy })),
  }));
export const fist = (frames: Frame[]) =>
  frames.map((frame) => ({
    ...frame,
    ...Object.fromEntries(
      (["l", "r"] as const).map((side) => [
        side,
        frame[side].map((p, k) =>
          k === 0 || k === 5 || k === 9 || k === 13 || k === 17
            ? p
            : { ...p, x: frame[side][9].x, y: frame[side][9].y },
        ),
      ]),
    ),
  }));
test("report reference and controlled perturbation evidence", () => {
  const rows = fixtures.map((ref) => {
    const variants = {
      self: ref.frames,
      translated: transform(ref.frames, 1, 0.08, -0.04),
      scaled: transform(ref.frames, 0.75, 0.08, 0.08),
      reversed: reverse(ref.frames),
      stationary: still(ref.frames),
      fist: fist(ref.frames),
      down: shiftHands(ref.frames, 0.16),
      missingLeft: ref.frames.map((f) => ({ ...f, l: [] })),
      missingRight: ref.frames.map((f) => ({ ...f, r: [] })),
    };
    const results = Object.fromEntries(
      Object.entries(variants).map(([name, attempt]) => {
        const v = evaluate(ref.frames, attempt);
        return [
          name,
          {
            ok: v.ok,
...
const mutateAll = (fn: (f: Frame) => void) =>
  copy(first).map((f) => {
    fn(f);
    return f;
  });
...
describe.each(fixtures)("$label real reference", (ref) => {
  test("self comparison is exact and leaves inputs unchanged", () => {
    ...
    expect(actual).toMatchObject({
      ok: true,
      score: 100,
      metrics: { shape: 100, location: 100, movement: 100 },
      coverage: 1,
    });
    expect(actual.path?.[0]).toEqual([0, 0]);
    expect(actual.path?.at(-1)).toEqual([
      ref.frames.length - 1,
      ref.frames.length - 1,
    ]);
  ...
  test("body translation and uniform scale preserve geometry", () => {
    for (const [scale, dx, dy] of [
      ...
      expect(
        evaluate(ref.frames, transform(ref.frames, scale, dx, dy)).score,
      ).toBe(100);
  });
  test("equivalent coordinates at a different image aspect preserve geometry", () => {
    const changed = ref.frames.map((frame) => ({
      ...frame,
      aspect: frame.aspect * 1.5,
      ...Object.fromEntries(
        (["p", "l", "r"] as const).map((s) => [
          s,
          frame[s].map((p) => ({ ...p, x: p.x / 1.5 })),
        ]),
      ),
    }));
  ...
  test("stationary and reversed geometry reduce the movement metric", () => {
    expect(
      evaluate(ref.frames, still(ref.frames)).metrics!.movement,
    ).toBeLessThan(50);
    expect(
      evaluate(ref.frames, reverse(ref.frames)).metrics!.movement,
    ).toBeLessThan(100);
  });
  test("altering finger geometry lowers shape even with matching wrists", () => {
    ...
    expect(
      changed.feedback.some((s) => s.includes("apertura de los dedos")),
    ).toBe(true);
  });
  test.each(["l", "r"] as const)(
    "missing %s hand abstains even if it is stationary in the reference",
    (side) => {
      expect(
        evaluate(
          ref.frames,
          ref.frames.map((f) => ({ ...f, [side]: [] })),
        ),
      ).toMatchObject({ ok: false, coverage: 0 });
    },
  );
...
describe("invalid and missing input", () => {
  test.each([NaN, Infinity, -Infinity, 0, -1])(
    "rejects invalid aspect %s",
    (aspect) => {
      const frames = mutateAll((f) => {
        f.aspect = aspect;
      });
  ...
  test("rejects unordered, repeated, and missing timestamps", () => {
    ...
    const repeated = copy(first);
    repeated[4].t = repeated[3].t;
    abstains(repeated);
    const absent = copy(first);
    delete (absent[4] as Partial<Frame>).t;
    abstains(absent);
  });
  test("rejects empty and very short input", () => {
    abstains([]);
    abstains(first.slice(0, 1));
    abstains(first.slice(0, 9));
    expect(evaluate(first.slice(0, 7), first).ok).toBe(false);
  });
  test.each([0, 11, 12, 13, 14])(
    "rejects absent pose anchor %s without throwing",
    (index) => {
      abstains(
        mutateAll((f) => {
          delete f.p[index];
  ...
  test.each(["x", "y", "z"] as const)(
    "rejects nonfinite pose and hand %s coordinates",
    (coordinate) => {
      for (const invalid of [NaN, Infinity, -Infinity]) {
        abstains(
          mutateAll((f) => {
            f.p[11][coordinate] = invalid;
          }),
        );
        abstains(
          mutateAll((f) => {
            f.l[20][coordinate] = invalid;
          }),
        );
        abstains(
          mutateAll((f) => {
            f.r[0][coordinate] = invalid;
          }),
        );
      }
    },
  ...
  test("rejects sparse, absent and collapsed hands", () => {
    ...
    abstains(
      mutateAll((f) => {
        f.r = undefined as unknown as Frame["r"];
      }),
    );
    abstains(
      mutateAll((f) => {
    ...
    abstains(
      mutateAll((f) => {
        f.l = Array.from({ length: 21 }, () => ({ ...f.l[0] }));
      }),
    );
  });
  test("rejects low/nonfinite required pose visibility", () => {
    for (const visibility of [0, 0.4, NaN, Infinity])
      abstains(
        mutateAll((f) => {
          f.p[0].visibility = visibility;
        }),
      );
  });
  test("accepts MediaPipe hand visibility zero defaults", () => {
    const attempt = mutateAll((f) => {
      for (const p of [...f.l, ...f.r]) p.visibility = 0;
    });
    expect(evaluate(first, attempt).score).toBe(100);
  });
  test("rejects nonfinite hand visibility", () =>
    abstains(
      mutateAll((f) => {
        f.l[0].visibility = NaN;
      }),
    ));
  ...
  test("handles malformed runtime frames without throwing", () => {
    ...
    abstains(
      mutateAll((f) => {
        f.p = undefined as unknown as Frame["p"];
      }),
    );
...
describe("coverage and temporal correspondence", () => {
  test("tolerates an isolated dropped detection and returns original input indices", () => {
    const attempt = copy(first);
    attempt[22].l = [];
    const reference = copy(first);
    reference[17].r = [];
  ...
  test("abstains across a long central or leading outage despite majority coverage", () => {
    for (const [start, end] of [
      ...
      const attempt = copy(first);
      for (let i = start; i < end; i++) attempt[i].r = [];
      const result = evaluate(first, attempt);
      expect(result.coverage).toBeGreaterThan(0.55);
  ...
  test("uses elapsed time for nonuniform capture density", () => {
    const dense = first.flatMap((f, i) =>
      i < first.length / 2
        ? [structuredClone(f), { ...structuredClone(f), t: f.t + 1 }]
        : [structuredClone(f)],
    );
  ...
  test("vertical feedback uses image y and names the anatomical hand", () => {
    const down = evaluate(first, shiftHands(first, 0.16));
    const up = evaluate(first, shiftHands(first, -0.16));
    expect(down.feedback.some((s) => s.includes("subir un poco la mano"))).toBe(
      true,
    );
    expect(up.feedback.some((s) => s.includes("bajar un poco la mano"))).toBe(
      true,
    );
    const rightOnly = copy(first).map((f) => ({
      ...f,
      r: f.r.map((p) => ({ ...p, y: p.y + 0.3 })),
    }));
    expect(
      evaluate(first, rightOnly).feedback.some((s) =>
        s.includes("subir un poco la mano derecha"),
      ),
    ).toBe(true);
  });
  test("a low score means a completed comparison, with corrective feedback and no success claim", () => {
    ...
    expect(
      result.feedback.some((s) => s.includes("trayectoria completa")),
    ).toBe(true);
    expect(
      result.feedback.some((s) =>
        s.startsWith("Los puntos observados se parecen"),
      ),
    ).toBe(false);
    expect(result.feedback).toContain(
      "Esta similitud visual no confirma que la seña sea correcta en LSC.",

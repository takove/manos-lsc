...
export type Evaluation = {
  ok: boolean;
  score?: number;
  metrics?: { shape: number; location: number; movement: number };
  feedback: string[];
  path?: [number, number][];
  coverage: number;
};
...
function body(f: Frame) {
  ...
  return {
    x: (a.x + b.x) / 2,
    y: (a.y + b.y) / 2,
    s: Math.hypot((a.x - b.x) * f.aspect, a.y - b.y),
  };
}
export function validPose(f: Frame) {
  if (
    !f ||
    !Number.isFinite(f.aspect) ||
    f.aspect <= 0 ||
    !Number.isFinite(f.t) ||
    f.t < 0 ||
    !Array.isArray(f.p) ||
  ...
  if (
    ![0, 11, 12, 13, 14].every(
      (i) => finitePoint(f.p[i]) && (f.p[i].visibility ?? 1) > 0.4,
    )
  )
    return false;
  const b = body(f);
  return (
    ...
    Number.isFinite(b.y) &&
    Number.isFinite(b.s) &&
    Math.abs(f.p[11].x - f.p[12].x) * f.aspect > 0.08
  );
}
...
export function position(f: Frame, h: Point[]) {
  if (!validPose(f) || !Array.isArray(h) || !finitePoint(h[0]))
    return [NaN, NaN];
  const b = body(f);
  return [((h[0].x - b.x) * f.aspect) / b.s, (h[0].y - b.y) / b.s];
}
function palmSize(h: Point[], aspect: number) {
  return Math.hypot((h[9].x - h[0].x) * aspect, h[9].y - h[0].y);
}
function present(h: Point[], aspect: number) {
  ...
  if (!Array.isArray(h) || h.length !== 21 || !Array.from(h).every(finitePoint))
    return false;
  ...
  return (
    ...
    h.every(
      (p) =>
        Number.isFinite(((p.x - h[0].x) * aspect) / size) &&
        Number.isFinite((p.y - h[0].y) / size),
    )
  );
}
function needed(f: Frame) {
  return validPose(f) && sides.every((side) => present(f[side], f.aspect));
}
...
function visible(frames: Frame[]) {
  return frames
    .map((frame, index) => ({ frame, index }))
    .filter((x) => needed(x.frame));
}
...
function hasGap(frames: Frame[], good: IndexedFrame[]) {
  ...
  const limit = Math.max(250, (end - start) * 0.15);
...
function sample(frames: IndexedFrame[], n = 48) {
  ...
  const start = frames[0].frame.t,
    end = frames[frames.length - 1].frame.t;
  let j = 0;
  return Array.from({ length: n }, (_, i) => {
    const target = start + (i * (end - start)) / (n - 1);
    while (
      j + 1 < frames.length &&
      Math.abs(frames[j + 1].frame.t - target) <
        Math.abs(frames[j].frame.t - target)
    )
      j++;
    return frames[j];
  });
...
const ranges = (seq: number[][]) =>
  seq[0].map(
    (_, i) =>
      Math.max(...seq.map((p) => p[i])) - Math.min(...seq.map((p) => p[i])),
  );
const score = (d: number, scale: number) =>
  Math.round(100 * Math.exp(-d / scale));
const referenceFailure = (feedback: string): Evaluation => ({
  ok: false,
  coverage: 0,
  feedback: [feedback],
});
...
export function evaluate(reference: Frame[], attempt: Frame[]): Evaluation {
  if (!timeline(reference))
    return referenceFailure(
      "La referencia tiene tiempos inválidos. Prueba otra lección.",
    );
  const referenceVisible = visible(reference);
  if (
    referenceVisible.length < 8 ||
    coverageOf(reference, referenceVisible) < 0.55 ||
    hasGap(reference, referenceVisible)
  )
    return referenceFailure(
      "La referencia no permite ver el cuerpo y las dos manos durante toda la seña. Prueba otra lección.",
    );
  if (!timeline(attempt))
    return {
      ok: false,
      coverage: 0,
      feedback: [
        "No pude ordenar el movimiento en el tiempo. Repite la grabación.",
      ],
    };
  const live = visible(attempt),
    coverage = coverageOf(attempt, live);
  if (live.length < 10 || coverage < 0.55 || hasGap(attempt, live))
    return {
      ok: false,
      coverage,
      feedback: [
  ...
  const rr = sample(referenceVisible),
    aa = sample(live);
  const features = (frames: IndexedFrame[]) =>
    frames.map(({ frame }) =>
      sides.map((side) => ({
        location: position(frame, frame[side]),
        shape: shape(frame[side], frame.aspect),
      })),
    );
  const rf = features(rr),
    af = features(aa);
  ...
  const align = (seq: typeof rf) =>
    seq.map((hands) =>
      hands.flatMap((hand) => [
        ...hand.location.map((v) => v / (0.6 * Math.sqrt(2))),
        ...hand.shape.map((v) => v / (0.8 * Math.sqrt(42))),
      ]),
    );
  const alignment = dtw(align(rf), align(af));
  if (!alignment.path.length || !Number.isFinite(alignment.distance))
    return {
      ok: false,
      coverage,
      feedback: [
        "Los puntos del movimiento no se pudieron comparar. Repite la grabación.",
      ],
    };
  const errors = sides.map((_, side) => {
    ...
    for (const [i, j] of alignment.path) {
      ...
      movementError += rms(
        x.location.map((v, k) => v - rf[0][side].location[k]),
        y.location.map((v, k) => v - af[0][side].location[k]),
      );
    ...
    movementError /= count;
    dy /= count;
    const referenceExtent = ranges(rf.map((h) => h[side].location)).reduce(
      (a, b) => a + b,
      0,
    );
    const attemptExtent = ranges(af.map((h) => h[side].location)).reduce(
      (a, b) => a + b,
      0,
    );
    if (referenceExtent > 0.25 && attemptExtent < referenceExtent * 0.25)
      movementError += 0.8;
  ...
  const metrics = {
    shape: score(Math.max(...errors.map((e) => e.shapeError)), 0.8),
    location: score(Math.max(...errors.map((e) => e.locationError)), 0.6),
    movement: score(Math.max(...errors.map((e) => e.movementError)), 0.6),
  };
  const overall = Math.round(
    metrics.shape * 0.45 + metrics.location * 0.3 + metrics.movement * 0.25,
  );
  ...
  if (metrics.location < 75) {
    ...
    feedback.push(
      dy > 0.15
        ? `Prueba subir un poco la ${hand} para acercarte a la altura del ejemplo.`
        : dy < -0.15
          ? `Prueba bajar un poco la ${hand} para acercarte a la altura del ejemplo.`
          : `Mira dónde empieza y termina la ${hand} respecto a tus hombros.`,
    );
  }
  if (metrics.shape < 75)
    feedback.push(
      "Compara la apertura de los dedos y la dirección de la mano con el video.",
    );
  if (metrics.movement < 75)
    feedback.push(
      "Repite la trayectoria completa: observa el punto de inicio, la dirección y el final.",
    );
  if (!feedback.length)
    feedback.push(
  ...
  feedback.push(
    "Esta similitud visual no confirma que la seña sea correcta en LSC.",
  );
  return {
    ok: true,
    score: overall,
    ...
    path: alignment.path.map(([i, j]) => [rr[i].index, aa[j].index]),
    coverage,
  };
}
